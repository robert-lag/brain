...
impl NoteMetadata {
    pub fn get_basic_data_of_file<P: AsRef<Path>>(file_path: P) -> Result<Note, String> {
        ...
        if let Some(note_id) = note_metadata[NoteProperty::NoteId.to_metadata_identifier().as_str()].as_str() {
        ...
        if let Some(note_name) = note_metadata[NoteProperty::NoteName.to_metadata_identifier().as_str()].as_str() {
        ...
        if let Some(creation_timestamp) = note_metadata[NoteProperty::CreationDate.to_metadata_identifier().as_str()].as_str() {
            if let Ok(creation_date_time) = Local.datetime_from_str(&creation_timestamp, "%Y-%m-%d %H:%M:%S") {
        ...
        if let Some(os_file_name) = &file_path.as_ref().file_name() {
            if let Some(file_name) = os_file_name.to_str() {
                ...
                return Err(format!("get-note-metadata: The file name '{}' contains illegal characters!",
                    &file_path.as_ref().to_string_lossy()));
            };
        } else {
            return Err(format!("get-note-metadata: Cannot get filename from the path '{}'!",
                &file_path.as_ref().to_string_lossy()));
        };
    ...
    pub fn get_property_of(note: &Note, property: NoteProperty, settings: &Settings) -> Result<Option<String>, String> {
    ...
    pub fn get_property_of_file<P: AsRef<Path>>(file_path: P, property: NoteProperty) -> Result<Option<String>, String> {
        let note_metadata = match NoteMetadata::get_metadata_of_file(file_path) {
            Ok(value) => value,
            Err(error) => {
                return Err(error);
    ...
    fn get_metadata_of_file<P: AsRef<Path>>(file_path: P) -> Result<Yaml, String> {
        let yaml_string = match NoteMetadata::get_yaml_header_of_file(&file_path) {
            Ok(header) => header,
            Err(error) => {
                return Err(format!("get-metadata: couldn't read header of note file '{}': {}",
                    &file_path.as_ref().to_string_lossy(), error));
            }
        };
        let note_metadata = match YamlLoader::load_from_str(&yaml_string) {
            Ok(yaml_vector) => yaml_vector[0].clone(),
            Err(error) => {
                return Err(format!("couldn't parse yaml header of file '{}': '{}'",
                    &file_path.as_ref().to_string_lossy(), error));
            }
        };
